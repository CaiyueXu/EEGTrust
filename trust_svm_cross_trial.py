...
if __name__ == '__main__':
    ...
    os.makedirs("./tmp_out/trust_svm/sub16_session", exist_ok=True)

    logger = logging.getLogger('svm_sub16')
    logger.setLevel(logging.DEBUG)
    console_handler = logging.StreamHandler()
    file_handler = logging.FileHandler('./tmp_out/trust_svm/sub16_session/svm_sub16.log')
    ...
    train_dataset = NumpyDataset(X=X_train,
                        y=y_train,
                        io_path=f'./tmp_out/trust_svm/sub16_session/train',
    ...
    test_dataset = NumpyDataset(X=X_test,
                        y=y_test,
                        io_path=f'./tmp_out/trust_svm/sub16_session/test',
                        # offline_transform=transforms.Compose(
                        #     [transforms.Concatenate([
                        #         transforms.BandDifferentialEntropy(),
                        #     transforms.BandPowerSpectralDensity()
    ...
    # label (int)
    # k_fold = KFold(n_splits=10, split_path=f'./tmp_out/trust_svm/sub05/split', shuffle=True)
    # torch.cuda.device(1)
