...
if __name__ == '__main__':
    ...
    os.makedirs("./tmp_out/trust_ccnn/sub08", exist_ok=True)

    logger = logging.getLogger('ccnn_sub08')
    logger.setLevel(logging.DEBUG)
    console_handler = logging.StreamHandler()
    file_handler = logging.FileHandler('./tmp_out/trust_ccnn/sub08/ccnn_sub08.log')
    logger.addHandler(console_handler)
    logger.addHandler(file_handler)
    Writer = SummaryWriter('./tmp_out/trust_ccnn/sub08')
    ...
    dataset = NumpyDataset(X=X,
                        y=y,
                        io_path=f'./tmp_out/trust_ccnn/sub08/deta',
    ...
    k_fold = KFold(n_splits=10, split_path=f'./tmp_out/trust_ccnn/sub08/split', shuffle=True)
    ...
    for i, (train_dataset, test_dataset) in enumerate(k_fold.split(dataset)):
        ...
        optimizer = torch.optim.Adam(model.parameters(), lr=1e-4, weight_decay = 1e-4)  # official: weight_decay=5e-1
        ...
        train_dataset, val_dataset = train_test_split(train_dataset,
                                                              test_size=0.1,#测试集比例
                                                              split_path=f'./tmp_out/trust_ccnn/sub08/split{i}',#第i次分割，会训练出第i个模型
                                                              shuffle=True)
        ...
        for t in range(epochs):#每个epoch也就是遍历了所有样本，对比并更新一下最优的模型，每个分割中通过50个epoch的训练保存一个最佳模型
            ...
            if val_acc > best_val_acc:
                ...
                torch.save(model.state_dict(), f'./tmp_out/trust_ccnn/sub08/model{i}.pt')
        #加载测试集
        test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=True)
        #加载训练好的模型，也就是截止目前，这个数据分割情况下最优的模型
        model.load_state_dict(torch.load(f'./tmp_out/trust_ccnn/sub08/model{i}.pt'))
        #测试结果
    ...
    np.savetxt("./tmp_out/trust_ccnn/sub08/fprs.csv", mean_fpr, delimiter=',')
    np.savetxt("./tmp_out/trust_ccnn/sub08/tprs_std.csv", std_tpr, delimiter=',')
    np.savetxt("./tmp_out/trust_ccnn/sub08/tprs.csv", mean_tpr, delimiter=',')
    np.savetxt("./tmp_out/trust_ccnn/sub08/aucs.csv", aucs, delimiter=',')
    Writer.close()
