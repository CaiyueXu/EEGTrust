...
if __name__ == '__main__':
    ...
    os.makedirs("./tmp_out/trust_knn/sub16_session", exist_ok=True)

    logger = logging.getLogger('knn_sub16')
    logger.setLevel(logging.DEBUG)
    console_handler = logging.StreamHandler()
    file_handler = logging.FileHandler('./tmp_out/trust_knn/sub16_session/knn_sub16.log')
    ...
    # sub04
    # X_test = np.concatenate((X[60*4:60*5-1,:,:], X[60*11:60*13-1,:,:], X[60*15:60*16-1,:,:], X[60*20:60*22-1,:,:]),axis=0)
    # y_test = {'trust': np.concatenate((y4[60*4:60*5-1], y4[60*11:60*13-1], y4[60*15:60*16-1], y4[60*20:60*22-1]),axis=0)}
    # X_train = np.concatenate((X[60*0:60*4-1,:,:], X[60*5:60*11-1,:,:], X[60*13:60*15-1,:,:], X[60*16:60*20-1,:,:], X[60*22:,:,:]),axis=0)
    # y_train = {'trust': np.concatenate((y4[60*0:60*4-1], y4[60*5:60*11-1], y4[60*13:60*15-1], y4[60*16:60*20-1], y4[60*22:]),axis=0)}
    # sub09
    ...
    train_dataset = NumpyDataset(X=X_train,
                        y=y_train,
                        io_path=f'./tmp_out/trust_knn/sub16_session/train',
    ...
    test_dataset = NumpyDataset(X=X_test,
                        y=y_test,
                        io_path=f'./tmp_out/trust_knn/sub16_session/test',
                        # offline_transform=transforms.Compose(
                        #     [transforms.Concatenate([
                        #         transforms.BandDifferentialEntropy(),
                        #     transforms.BandPowerSpectralDensity()
